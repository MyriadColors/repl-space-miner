...
def _display_trading_header(game_state: Game, station: Station, player_character) -> None:
    ...
    cargo_color = Fore.GREEN if cargo_percentage < 80 else Fore.YELLOW if cargo_percentage < 95 else Fore.RED
    game_state.ui.info_message(
...
def _enhanced_buy_interface(game_state: Game) -> None:
    ...
    while True:
        ...
        if not station.ore_cargo or all(cargo.quantity <= 0 for cargo in station.ore_cargo):
            game_state.ui.error_message(
        ...
        available_items = [
            cargo for cargo in station.ore_cargo if cargo.quantity > 0]
        ...
        game_state.ui.info_message(
            "• Type 'all' to buy maximum affordable quantity")
...
def _trade_routes_interface(game_state: Game) -> None:
    ...
    include_unreachable = take_input(
        "Include unreachable stations? (y/n, default n): ").lower() == "y"
...
def _quick_trade_interface(game_state: Game) -> None:
    ...
    game_state.ui.info_message(
        "2. Buy maximum affordable of most profitable item")
...
def _trading_statistics_interface(game_state: Game) -> None:
    ...
    for cargo in player_ship.cargohold:
        ...
        if station_ore:
            estimated_value = _calculate_modified_sell_price(
                game_state, station_ore.sell_price) * cargo.quantity
        else:
    ...
    cargo_efficiency = (total_cargo_volume / player_ship.cargohold_capacity *
                        100) if player_ship.cargohold_capacity > 0 else 0
    ...
    trader_rep_bonus = player_character.reputation_traders * \
        0.25 if hasattr(player_character, 'reputation_traders') else 0

    game_state.ui.info_message(
        f"Charisma Bonus: {charisma_bonus:.1f}% better prices")
    game_state.ui.info_message(
    ...
    for cargo in player_ship.cargohold:
        ...
        if station_ore:
            ...
            profit_margin = (sell_price - cargo.ore.base_value) / \
                cargo.ore.base_value if cargo.ore.base_value > 0 else 0
            ore_name = f"{cargo.ore.purity.name} {cargo.ore.name}"
            best_sells.append(
...
def _smart_cargo_management(game_state: Game) -> None:
    ...
    for cargo in player_ship.cargohold:
        ...
        if station_ore:
            sell_price = _calculate_modified_sell_price(
                game_state, station_ore.sell_price)
            value_per_volume = sell_price / cargo.ore.volume if cargo.ore.volume > 0 else 0
            ore_name = f"{cargo.ore.purity.name} {cargo.ore.name}"
    ...
    for ore_cargo in station.ore_cargo:
        if ore_cargo.quantity > 0:
            buy_price = _calculate_modified_buy_price(
                game_state, ore_cargo.buy_price)
            value_per_volume = ore_cargo.ore.base_value / \
                ore_cargo.ore.volume if ore_cargo.ore.volume > 0 else 0
            ore_name = f"{ore_cargo.ore.purity.name} {ore_cargo.ore.name}"
    ...
    game_state.ui.info_message(
        "Least efficient cargo items (candidates for selling):")
    ...
    for i, (ore_cargo, ore_name, buy_price, efficiency) in enumerate(station_efficiency[:3]):
        game_state.ui.info_message(
...
def _process_item_sale(game_state: Game, ore_cargo) -> None:
    ...
    game_state.ui.info_message(
        f"Estimated Total: {estimated_total:.2f} credits")
...
def _sell_all_cargo(game_state: Game) -> None:
    ...
    confirm = take_input(
        f"{Fore.YELLOW}Sell all cargo? (y/n): {Style.RESET_ALL}")
...
def _buy_all_affordable(game_state: Game, available_items) -> None:
    ...
    for i, ore_cargo in enumerate(available_items, 1):
        ore_name = f"{ore_cargo.ore.purity.name} {ore_cargo.ore.name}"
        unit_price = _calculate_modified_buy_price(
            game_state, ore_cargo.buy_price)
        player_character = game_state.get_player_character()
        max_affordable = int(player_character.credits // unit_price)
        max_available = min(max_affordable, ore_cargo.quantity)
        ...
        game_state.ui.info_message(
            f"{i}. {ore_name} - Max affordable: {max_available}")
...
def _buy_most_profitable(game_state: Game) -> None:
    ...
    opportunities = get_trade_opportunities(
        game_state, include_unreachable=False)
    ...
    for ore_name, buy_station, sell_station, buy_price, sell_price, profit_per_unit, is_viable in opportunities:
    ...
    if not available_opportunities:
        game_state.ui.warn_message(
            "No profitable items available at this station.")
        take_input("Press Enter to continue...")
        return
    ...
    modified_price = _calculate_modified_buy_price(
        game_state, ore_cargo.buy_price)
...
def _fill_cargo_cheap(game_state: Game) -> None:
    ...
    available_items = [(ore_cargo, _calculate_modified_buy_price(game_state, ore_cargo.buy_price))
                       for ore_cargo in station.ore_cargo if ore_cargo.quantity > 0]
    ...
    game_state.ui.info_message(
        f"Available cargo space: {cargo_space_available:.2f} m³")
    ...
    game_state.ui.info_message(
        f"Remaining Cargo Space: {remaining_space:.2f} m³")
