...
def refine_command(game_state: Game, amount: Optional[int] = None) -> None:
    ...
    if amount is not None and isinstance(amount, str):
        try:
            ...
            game_state.ui.error_message("Invalid amount. Please provide a valid number.")
            return
    ...
    refinable_cargo = [cargo for cargo in player_ship.cargohold if cargo.ore.can_refine()]
    ...
    try:
        ...
        if selection < 1 or selection > len(refinable_cargo):
            game_state.ui.error_message(f"Invalid selection. Please choose 1-{len(refinable_cargo)}.")
            return
        ...
        if amount is None or amount <= 0:
            try:
                amount_str = take_input(f"How many units to refine (1-{max_amount}, 'all' for all): ")
        ...
        refining_cost_per_unit = base_refining_cost * selected_cargo.ore.refining_difficulty
        ...
        game_state.ui.info_message("\n=== Refining Summary ===")
        game_state.ui.info_message(f"Ore: {selected_cargo.ore.purity.name} {selected_cargo.ore.name} -> {refined_ore.purity.name} {refined_ore.name}")
        ...
        game_state.ui.info_message(f"Net profit: {value_increase - total_refining_cost} credits")
        ...
        for cargo in player_ship.cargohold:
            if cargo.ore.id == selected_cargo.ore.id and cargo.ore.purity == refined_ore.purity:
...
def refine_to_minerals_command(game_state: Game, amount: Optional[int] = None) -> None:
    ...
    # Must be docked at a station to refine
    if not player_ship.is_docked:
        game_state.ui.error_message("You must be docked at a station to refine ore into minerals.")
        return
    ...
    # Display available items to refine
    refinable_cargo = [cargo for cargo in player_ship.cargohold if cargo.ore.mineral_yield]
    
    if not refinable_cargo:
        game_state.ui.error_message("You have no ore that can be refined into minerals.")
        return
    ...
    # Get user selection
    try:
        selection = int(take_input("Select ore number to refine into minerals (0 to cancel): "))
        if selection == 0:
            game_state.ui.info_message("Refining cancelled.")
            return
        ...
        skill_bonus = 1.0
        if hasattr(player_character, 'engineering') and player_character.engineering > 5:
            skill_bonus = 1.0 + ((player_character.engineering - 5) * 0.01)
        
        # Show refining summary
        game_state.ui.info_message("\n=== Mineral Refining Summary ===")
        game_state.ui.info_message(f"Ore: {selected_cargo.ore.purity.name} {selected_cargo.ore.name}")
        game_state.ui.info_message(f"Amount: {amount} units")
        game_state.ui.info_message(f"Engineering Skill Bonus: +{(skill_bonus - 1.0) * 100:.0f}%")
        ...
        game_state.ui.info_message(f"Total minerals value: {total_minerals_value} credits")
        ...
        game_state.ui.info_message(f"Refining cost: {total_refining_cost} credits")
        game_state.ui.info_message(f"Net profit: {total_minerals_value - total_refining_cost} credits")
        ...
        for mineral_id, mineral_amount in minerals_produced.items():
            mineral = MINERALS.get(mineral_id)
            if mineral and mineral_amount > 0:                # Check if we already have this mineral in inventory
                ...
                for cargo in player_ship.mineralhold:
                    if cargo.mineral.id == mineral_id and cargo.mineral.quality == MineralQuality.STANDARD:
                ...
                if existing_mineral_cargo:
                    ...
                    buy_price = mineral.get_value() * 0.8  # Some markdown from base value
                    sell_price = mineral.get_value() * 1.2  # Some markup from base value
        ...
        assert amount is not None, "Amount should not be None here"
        skill_results = process_skill_xp_from_activity(game_state, "Refining & Processing", amount * 3)
        notify_skill_progress(game_state, skill_results)
        
        mineral_names = [MINERALS[mineral_id].name for mineral_id in minerals_produced.keys()]
