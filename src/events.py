...
def intro_event(game_state: 'Game'):
    ...
    print(Fore.GREEN + "- Younger (18-30): Better reflexes (+1 Piloting), less reputation (-5 all factions)")
    print(Fore.GREEN + "- Middle-aged (31-45): Balanced abilities, established contacts (+5 all factions)")
    print(Fore.GREEN + "- Older (46-60): Slower reflexes (-1 Piloting), unparalleled experience (+1 Education) and reputation (+10 all factions)")
        ...
        if is_valid_int(age_input):
            ...
            if 18 <= age <= 60:
                if 18 <= age <= 30:
                    ...
                    print(Fore.CYAN + "Your balanced experience serves you well. Reputation bonus applied.")
                else:
                    age_category = "veteran"
                    age_bonuses = {"piloting": -1, "education": 1, "faction_modifier": 10}
                    print(Fore.CYAN + "Your reputation precedes you. Respect and knowledge bonuses applied.")
                break
    ...
    while remaining_points > 0:
        ...
        skill_to_improve = input("Which skill would you like to improve? (piloting/engineering/combat/education/charisma): ").lower()
        ...
        try:
            points = int(input(f"How many points to add to {skill_to_improve}? (1-{remaining_points}): "))
            if 1 <= points <= remaining_points:
                base_skills[skill_to_improve] += points
                remaining_points -= points
                print(Fore.GREEN + f"{skill_to_improve.capitalize()} is now {base_skills[skill_to_improve]}")
            else:
                print(Fore.RED + f"Please enter a number between 1 and {remaining_points}.")
        except ValueError:
            print(Fore.RED + "Invalid input. Please enter a number.")
    ...
    try:
        ...
        print(Fore.YELLOW + "Note: Character traits recorded but will need game update to take effect.")
    ...
    for idx, choice in enumerate(ship_choices.keys(), 1):
        ...
        print(f"   Hull Integrity: {ship_data['hull_integrity']}, Shield Capacity: {ship_data['shield_capacity']}")
    ...
    print(Fore.GREEN + "Modifications will improve your ship but increase your starting debt.")
    print(Fore.GREEN + "You can choose to skip modifications for a harder start but lower debt.")
    ...
    if mod_decision.lower() in ["y", "yes"]:
        ...
        ship_mods = [
            {"name": "Enhanced Sensors", "desc": "Longer detection range (+0.25 AU)", "cost": 5000, "stat": "sensor_range", "value": 0.25},
            {"name": "Hidden Cargo Compartments", "desc": "Extra 'unofficial' cargo space (+20 units)", "cost": 3000, "stat": "cargo_capacity", "value": 20.0},
            {"name": "Reinforced Hull", "desc": "Better damage resistance (+20% hull)", "cost": 4000, "stat": "hull_integrity", "value": 20.0},
            {"name": "Engine Tuning", "desc": "Improved speed (+10% speed)", "cost": 4500, "stat": "speed", "value": 0.1, "multiplier": True},
            {"name": "Efficient Fuel System", "desc": "Reduced consumption (-10% fuel use)", "cost": 3500, "stat": "fuel_consumption", "value": -0.1, "multiplier": True},
            {"name": "Mining Laser Upgrade", "desc": "Faster resource extraction (+15% mining)", "cost": 4000, "stat": "mining_speed", "value": 0.15, "multiplier": True}
        ]
        
        for i, mod in enumerate(ship_mods, 1):
            print(Fore.GREEN + f"{i}. {mod['name']}: {mod['desc']} (+{mod['cost']} credits debt)")
            ...
            try:
                ...
                if 1 <= mod_choice_num <= len(ship_mods):
                    ...
                    if selected_mod in selected_mods:
                        print(Fore.RED + "You've already selected this modification. Choose another.")
                        continue
                    ...
                    if isinstance(mod_cost, (int, float)):
                        ...
                        debt_increase += 0
                    print(Fore.CYAN + f"{selected_mod['name']} installed. {selected_mod['desc']}")
                    num_selections += 1
                    
                    # After first mod, ask if they want another or if they're done
                    if num_selections == 1:
                        print(Fore.GREEN + "You can select one more modification, or type 'done' to finish.")
                else:
                    print(Fore.RED + f"Invalid choice. Please enter a number between 1 and {len(ship_mods)}.")
            except ValueError:
                print(Fore.RED + "Invalid input. Enter a number or 'done'.")
                
        if not selected_mods:
            print(Fore.CYAN + "You chose not to add any modifications. The ship remains in its basic configuration.")
        elif len(selected_mods) == 1:
            print(Fore.CYAN + "You decided on a single modification. A wise, economical choice.")
    else:
        print(Fore.CYAN + "You decided to save your credits and avoid additional debt. The ship remains in its basic configuration.")
        selected_mods = []
        debt_increase = 0
    ...
    print(Fore.YELLOW + f"Total debt increased by {debt_increase} credits for modifications.")
    ...
    if chosen_background.name == "Ex-Miner":
        ...
        mission_desc = "'I need certain rare minerals that only you'd know how to identify.'"
    elif chosen_background.name == "Corp Dropout" or chosen_background.name == "Void Runner":
        mission_type = "data courier" 
        mission_desc = "'Deliver this data cache. Corporate eyes are watching, so be discreet.'"
    elif chosen_background.name == "Xeno-Biologist":
        mission_type = "specimen retrieval"
        mission_desc = "'A biological sample needs collection. Your expertise will be crucial.'"
    else:
        mission_type = "package delivery"
        mission_desc = "'Transport this package to Outpost 7. Simple enough, even for you.'"
    ...
    if q_choice == 1:
        ...
        if character.charisma >= 8:
            print(Fore.CYAN + f"Your charisma impresses {contact_name}. 'Fine. 5500 credits.'")
            bonus_pay = 500
        else:
            bonus_pay = 0
    elif q_choice == 3:
    ...
    print(f"Ship: {ship_name} ({ship_choices[chosen_ship]['specialty']} specialization)")
