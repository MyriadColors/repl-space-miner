...
def display_time_and_status(game_state: Game) -> None:
    ...
    game_state.ui.info_message(f"Current game time: {format_seconds(game_state.global_time)}")
...
def display_help(game_state: Game, command_name: str = "") -> None:
    ...
    def write_command(command: str, description: str, allowed: bool = True, context_required: Optional[str] = None) -> None:
        ...
        is_in_field, _ = player_ship.check_field_presence(game_state) if player_ship else (False, None)
        ...
        game_state.ui.info_message(f"{color}{command}{Style.RESET_ALL}: {description} {color}{status}{Style.RESET_ALL}")
    ...
    if command_name:
        ...
        if command:
            ...
            game_state.ui.info_message(f"Description: {command.function.__doc__ or 'No description available'}")
            if command.arguments:
                game_state.ui.info_message("\nArguments:")
                for arg in command.arguments:
                    optional = "(optional)" if arg.is_optional else "(required)"
                    game_state.ui.info_message(f"  {arg.name} ({arg.type.__name__}) {optional}")
        else:
        ...
        # Travel commands
        write_command("travel/t <x> <y>", "Travel to specific coordinates", True, "undocked")
        write_command("closest/c <field|station>", "Travel to closest field or station", True, "undocked")
        write_command("direct/d <x> <y>", "Direct travel to coordinates", True, "undocked")
        
        # Mining commands
        write_command("mine/m <time> [until_full] [ore]", "Mine asteroids for specified time", True, "field")
        ...
        # Upgrade commands
        write_command("upgrade/up [id]", "View or purchase ship upgrades", True, "docked")
