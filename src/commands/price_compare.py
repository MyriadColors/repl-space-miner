...
def get_travel_details(game_state: Game, station: Station) -> Tuple[float, float, float, bool]:
    """
...
def get_best_buy_prices(
    ...
    for station in current_system.stations:
        ...
        if player_ship.is_docked and player_ship.docked_at == station:
            ...
            distance, travel_time, fuel_consumed, is_reachable = get_travel_details(game_state, station)
        ...
        for ore_cargo in station.ore_cargo:
            ...
            total_cost = buy_price + (fuel_cost / 100)  # Amortize fuel cost over 100 units
...
def get_best_sell_prices(
    ...
    for station in current_system.stations:
        ...
        for ore_cargo in station.ore_cargo:
            ...
            net_profit = sell_price - (fuel_cost / 100)  # Amortize fuel cost over 100 units
...
def get_trade_opportunities(
    ...
    for ore_name in buy_prices:
        ...
        for buy_station, buy_price, buy_fuel_cost, _, buy_reachable in buy_prices[ore_name]:
            for sell_station, sell_price, sell_fuel_cost, _, sell_reachable in sell_prices[ore_name]:
                ...
                profit_per_unit = price_diff - (trip_fuel_cost / 100)  # Amortize fuel cost over 100 units
...
def compare_fuel_prices(game_state: Game, stations: List[Station], include_unreachable: bool) -> None:
    """
    ...
    for station in stations:
        ...
        if player_ship.is_docked and player_ship.docked_at == station:
            ...
            travel_cost_per_unit = fuel_cost / typical_refuel if typical_refuel > 0.0 else 0.0
    ...
    for station, distance, fuel_consumed, fuel_cost, price, effective_price, is_reachable in stations_with_data:
...
def compare_prices_command(game_state: Game, option: str = "all", show_all: str = "no") -> None:
    """
    ...
    game_state.ui.info_message(f"Current position: ({player_ship.space_object.position.x:.3f}, {player_ship.space_object.position.y:.3f})")
    game_state.ui.info_message(f"Fuel available: {player_ship.fuel:.2f}/{player_ship.max_fuel} m³")
    game_state.ui.info_message(f"Fuel consumption: {player_ship.fuel_consumption:.4f} m³/AU\n")
    ...
    if option in ["buy", "all"]:
        ...
        if buy_prices:
            ...
            game_state.ui.info_message(f"{'Ore':<20} {'Station':<20} {'Price':<10} {'Fuel Cost':<10} {'Reachable':<10}")
    ...
    if option in ["trade", "all"]:
        ...
        if opportunities:
            ...
            for ore_name, buy_station, sell_station, buy_price, sell_price, profit, viable in opportunities[:10]:
            ...
            if len(opportunities) > 10:
                game_state.ui.info_message(f"Showing top 10 of {len(opportunities)} opportunities.")
        else:
    ...
    game_state.ui.info_message("  Show all: yes/no - Include unreachable stations (default: no)")
...
def find_best_trade_routes(
    ...
    if include_unreachable is None:
        ...
        include_unreachable_bool = include_unreachable.lower() in ["true", "yes", "y", "1"]
    else:
    ...
    for ore_name in buy_prices:
        ...
        if buy_prices[ore_name]:
            ...
            ore_cargo = next((oc for oc in station.ore_cargo if f"{oc.ore.purity.name} {oc.ore.name}" == ore_name), None)
    ...
    for ore_name in buy_prices:
        if ore_name not in sell_prices or not buy_prices[ore_name] or not sell_prices[ore_name]:
            continue
        ...
        for buy_index, buy_data in enumerate(buy_prices[ore_name]):
            ...
            for sell_index, sell_data in enumerate(sell_prices[ore_name]):
                ...
                if buy_station == sell_station or (not sell_reachable and not include_unreachable_bool):
                    continue
                ...
                trip_distance = (
                    start_pos.distance_to(buy_station.position) + 
                    buy_station.position.distance_to(sell_station.position)
                )
                
                trip_fuel = trip_distance * player_ship.fuel_consumption
                trip_fuel_cost = trip_fuel * buy_station.fuel_price  # Use buy station's fuel price as proxy
    ...
    if routes:
        game_state.ui.info_message(
            ...
            f"{'Units':<8} {'Dist':<8} {'Fuel Cost':<10} {'Total Profit':<12} {'Viable':<8}"
        )
        ...
        for i, route in enumerate(routes[:max_routes_int]):
            ...
            if route['total_profit'] > 10000:
                total_profit_str = f"{Fore.GREEN}{route['total_profit']:.2f}{Style.RESET_ALL}"
            elif route['total_profit'] > 5000:
                total_profit_str = f"{Fore.YELLOW}{route['total_profit']:.2f}{Style.RESET_ALL}"
        ...
        if len(routes) > max_routes_int:
            game_state.ui.info_message(f"Showing top {max_routes_int} of {len(routes)} routes.")
    else:
...
register_command(
    ...
    [
        Argument("option", str, True),
        Argument("show_all", str, True)
    ]
)
...
register_command(
    ...
    [
        Argument("max_routes", int, True),
        Argument("include_unreachable", bool, True)
    ]
)
