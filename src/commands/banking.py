...
class BankingTransaction:
    ...
    def __init__(self, transaction_type: str, amount: float, description: str = ""):
        ...
        self.transaction_type = transaction_type  # "deposit", "withdraw", "loan", "repayment", "interest"
        self.amount = amount
...
def banking_menu_command(game_state: Game) -> None:
    ...
    if not player_ship.is_docked:
        game_state.ui.error_message("You need to be docked at a station to access banking services.")
        return
    ...
    while True:
        ...
        game_state.ui.info_message(f"{Fore.CYAN}GALACTIC BANKING NETWORK - {station_name}{Style.RESET_ALL}")
        ...
        if choice == "1":
            ...
            game_state.ui.info_message("Thank you for using the Galactic Banking Network.")
            break
        else:
...
def display_financial_summary(game_state: Game, character) -> None:
    ...
    if character.savings > 0:
        game_state.ui.success_message(f"Savings Account: {character.savings:.2f} credits (Interest: {character.savings_interest_rate:.1%}/week)")
    else:
        game_state.ui.info_message(f"Savings Account: {character.savings:.2f} credits (Interest: {character.savings_interest_rate:.1%}/week)")
    ...
    if debt <= 0:
        ...
        game_state.ui.warn_message(f"Warning: High debt levels. Interest is accumulating rapidly!")
    elif debt > 5000:
        ...
        if game_state.global_time > 0 and character.credits > 0:
            ...
            if total_game_time_hours > 0:
                ...
                if credits_per_hour > 0:
                    ...
                    game_state.ui.info_message(f"At your current earning rate, you could pay off your debt in approximately {days_to_payoff:.1f} game days.")
...
def debt_management_menu(game_state: Game, character) -> None:
    ...
    if character.debt <= 0:
        ...
        input(Fore.YELLOW + "Press Enter to return to banking menu..." + Style.RESET_ALL)
        return
    ...
    while True:
        ...
        game_state.ui.info_message(f"3. Pay Minimum (Interest Only: {weekly_interest:.2f} credits)")
        ...
        if choice == "1":
            # Custom payment
            amount = get_valid_amount(game_state, "Enter amount to repay", 0, min(character.credits, character.debt))
            ...
            if character.credits >= character.debt:
                ...
                game_state.ui.error_message("You don't have enough credits to pay your debt in full.")
                time.sleep(1.5)
        elif choice == "3":
            ...
            if character.credits >= weekly_interest:
                ...
                game_state.ui.error_message("You don't have enough credits to make the minimum payment.")
                time.sleep(1.5)
        elif choice == "4":
            # Return to main menu
            break
...
def loan_menu(game_state: Game, character) -> None:
    ...
    while True:
        ...
        color = Fore.GREEN if credit_score > 700 else Fore.YELLOW if credit_score > 500 else Fore.RED
        game_state.ui.info_message(f"Credit Score: {color}{credit_score}{Style.RESET_ALL} ({credit_rating})")
        
        if max_loan > 0:
            game_state.ui.info_message(f"Maximum Loan Available: {max_loan:.2f} credits")
        else:
            game_state.ui.warn_message("You do not qualify for additional loans at this time.")
        ...
        if choice == "1":
            # Apply for loan
            if max_loan <= 0:
                game_state.ui.error_message("Your current credit rating does not qualify for a new loan.")
                time.sleep(1.5)
            else:
...
def savings_menu(game_state: Game, character) -> None:
    ...
    while True:
        ...
        game_state.ui.info_message(f"Interest Rate: {character.savings_interest_rate:.1%}/week")
        game_state.ui.info_message(f"Available Credits: {character.credits:.2f}")
        
        # Calculate projected interest
        weekly_interest = character.savings * character.savings_interest_rate
        game_state.ui.info_message(f"Projected Weekly Interest: +{weekly_interest:.2f} credits")
        ...
        if choice == "1":
            # Deposit funds
            amount = get_valid_amount(game_state, "Enter amount to deposit", 0, character.credits)
            if amount is not None:
                deposit_to_savings(game_state, character, amount)
        elif choice == "2":
            # Withdraw funds
            amount = get_valid_amount(game_state, "Enter amount to withdraw", 0, character.savings)
            if amount is not None:
...
def view_transaction_history(game_state: Game, character) -> None:
    ...
    if not hasattr(character, "bank_transactions") or len(character.bank_transactions) == 0:
        ...
        for transaction in reversed(character.bank_transactions[-20:]):  # Show last 20 transactions
            ...
            elif transaction.transaction_type == "withdraw" or transaction.transaction_type == "loan":
...
def repay_debt(game_state: Game, character, amount: float) -> None:
    ...
    # Confirm payment
    confirm = input(Fore.YELLOW + f"Confirm payment of {amount:.2f} credits toward your debt? (y/n): " + Style.RESET_ALL)
    if confirm.lower() != "y":
...
def calculate_credit_score(game_state: Game, character) -> int:
    ...
    if hasattr(character, "bank_transactions"):
        ...
        recent_transactions = character.bank_transactions[-10:] if len(character.bank_transactions) > 10 else character.bank_transactions
        repayments = [t for t in recent_transactions if t.transaction_type == "repayment"]
        repayment_bonus = min(100, len(repayments) * 20)  # Up to +100 points for consistent repayments
...
def apply_for_loan(game_state: Game, character, max_loan: float) -> None:
    ...
    # Confirm loan
    confirm = input(Fore.YELLOW + f"Do you accept these terms and wish to proceed with the loan? (y/n): " + Style.RESET_ALL)
    if confirm.lower() != "y":
    ...
    # Record the transaction
    transaction = BankingTransaction("loan", amount, f"New loan at {weekly_rate:.1%} interest")
    if not hasattr(character, "bank_transactions"):
        character.bank_transactions = []
    character.bank_transactions.append(transaction)
    
    game_state.ui.success_message(f"\nLoan approved! {amount:.2f} credits have been added to your account.")
    game_state.ui.info_message(f"Your new debt balance is {character.debt:.2f} credits.")
...
def display_loan_terms(game_state: Game, character) -> None:
    ...
    if character.debt_interest_mod != 1.0:
        if character.debt_interest_mod > 1.0:
            game_state.ui.warn_message(f"Your Personalized Rate: {adjusted_rate:.1%}/week (+{(character.debt_interest_mod-1)*100:.0f}%)")
            if character.negative_trait == "Indebted":
                game_state.ui.warn_message(f"Your 'Indebted' trait increases your interest rates.")
        else:
            game_state.ui.success_message(f"Your Personalized Rate: {adjusted_rate:.1%}/week (-{(1-character.debt_interest_mod)*100:.0f}%)")
    ...
    game_state.ui.info_message(f"- Failure to make regular payments may affect credit score")
    game_state.ui.warn_message(f"- High debt levels may attract attention from debt collectors")
...
def deposit_to_savings(game_state: Game, character, amount: float) -> None:
    ...
    # Confirm deposit
    confirm = input(Fore.YELLOW + f"Confirm deposit of {amount:.2f} credits to your savings account? (y/n): " + Style.RESET_ALL)
    if confirm.lower() != "y":
        game_state.ui.info_message("Transaction cancelled.")
        time.sleep(1)
        return
...
def withdraw_from_savings(game_state: Game, character, amount: float) -> None:
    ...
    # Confirm withdrawal
    confirm = input(Fore.YELLOW + f"Confirm withdrawal of {amount:.2f} credits from your savings account? (y/n): " + Style.RESET_ALL)
    if confirm.lower() != "y":
        game_state.ui.info_message("Transaction cancelled.")
        time.sleep(1)
        return
...
def get_valid_amount(game_state: Game, prompt: str, min_amount: float = 0, max_amount: float = float('inf')) -> Optional[float]:
    """Get a valid amount input from the user."""
    while True:
        amount_input = input(Fore.YELLOW + f"{prompt} (max {max_amount:.2f}): " + Style.RESET_ALL)
        ...
        if is_valid_float(amount_input):
            ...
            if amount <= 0:
                ...
                game_state.ui.error_message(f"Amount must be at least {min_amount:.2f}.")
            elif amount > max_amount:
                game_state.ui.warn_message(f"Amount exceeds maximum. Adjusting to {max_amount:.2f}.")
                return max_amount
            else:
                return amount
        else:
...
def calculate_savings_interest(game_state: Game, character) -> Optional[tuple[float, float]]:
    ...
    weeks_passed = (game_state.global_time - character.last_savings_interest_time) // WEEK_LENGTH
    ...
    if weeks_passed >= 1:
        ...
        # Record the transaction
        if total_interest > 0:
            transaction = BankingTransaction("interest", total_interest, "Savings interest payment")
            if not hasattr(character, "bank_transactions"):
                character.bank_transactions = []
            character.bank_transactions.append(transaction)
