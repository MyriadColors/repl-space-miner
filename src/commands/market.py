...
def market_command(game_state: Game) -> None:
    ...
    if not player_ship.is_docked:
        game_state.ui.error_message("You must be docked at a station to access the market.")
        return
    ...
    game_state.ui.info_message(f"Cargo Capacity: {station.ore_cargo_volume:.2f}/{station.ore_capacity:.2f} m³\n")
    
    # Display fuel prices
    game_state.ui.info_message("=== FUEL ===")
    game_state.ui.info_message(f"Fuel Price: {station.fuel_price:.2f} credits per m³")
    game_state.ui.info_message(f"Available Fuel: {station.fuel_tank:.2f}/{station.fuel_tank_capacity:.2f} m³\n")
    
    # Display available ores for trading
    game_state.ui.info_message("=== AVAILABLE ORES ===")
    game_state.ui.info_message(f"{"Ore":<20} {"Quantity":<10} {"Buy Price":<12} {"Sell Price":<12} {"Value Diff":<12}")
