...
def display_all_skills(game_state: Game, skill_system) -> None:
    ...
    game_state.ui.info_message(
        f"Unspent Skill Points: {skill_system.unspent_skill_points}"
    )
...
def display_skill_detail(game_state: Game, skill_system, skill_name: str) -> None:
    ...
    if not skill:
        ...
        if len(matches) == 1:
            ...
            game_state.ui.error_message(
                f"Multiple skills match '{skill_name}': {', '.join(matches)}"
            )
    ...
    game_state.ui.info_message(
        f"XP: {skill.xp} / {skill.xp_for_previous_level() + skill.xp_for_next_level()}"
    )
    ...
    if skill_system.unspent_skill_points > 0:
        ...
        game_state.ui.info_message(
            f"Type 'skills spend {skill_name}' to level up this skill."
        )
...
def spend_skill_point(game_state: Game, skill_system, skill_name: str) -> None:
    ...
    if success:
        game_state.ui.success_message(
            f"Successfully improved {skill_name.capitalize()} to level {skill.level}!"
        )
        game_state.ui.info_message(
            f"Remaining skill points: {skill_system.unspent_skill_points}"
        )
    else:
...
def register_skill_commands():
    register_command(
        ...
        skills_command,
        [
            Argument("args", str, True)
        ],  # Change from list to str to handle full skill name
    )
